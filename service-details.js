...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (serviceId && serviceData[serviceId]) {
        ...
        // Update Title
        if (heroTitle) heroTitle.innerText = service.title;
        document.title = `${service.title} - Fixapathway`;

        // Build Details List HTML
        const detailsList = service.details.map(item => `<li><i class="fas fa-check-circle" style="color: var(--primary-blue); margin-right: 10px;"></i>${item}</li>`).join('');
            ...
            contentDiv.innerHTML = `
                <div class="grid">
                    <div style="text-align: center; margin-bottom: 2rem;">
                        <img src="${service.icon}" alt="${service.title}" style="width: 150px; height: 150px; object-fit: contain; margin-bottom: 2rem;">
                        <h2 style="color: var(--primary-blue); margin-bottom: 1.5rem;">${service.title}</h2>
                        <p style="font-size: 1.2rem; max-width: 800px; margin: 0 auto; color: var(--midnight-blue);">${service.description}</p>
                    </div>
                </div>
                <div class="grid" style="margin-top: 3rem;">
                    ...
                    <article style="background: var(--primary-blue) !important; color: white !important; display: flex; flex-direction: column; justify-content: center; text-align: center;">
                        ...
                        <a href="contact" role="button" style="background: white; color: var(--primary-blue); border: none; font-weight: bold; margin-top: 1rem;">Start Recovery Now</a>
                    </article>
                </div>
            `;
        }
